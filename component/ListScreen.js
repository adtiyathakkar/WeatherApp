...
export default function ListScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    fetch("http://api.openweathermap.org/data/2.5/find?lat=23.68&lon=90.35&cnt=50&appid=46a26121c9bcd9a1fd5c9cb8903c9180", datafetch)
      ...
      .then(results => {
        ...
        dispatch({ type: "SET_WEATHER_DATA", payload: results.list })
      })
      .catch(err => {
        setIsLoading(false);
        setError(err);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        data={state.weatherData}//show list of city weather data using flatlist using context api (state.weatherdata) 
        keyExtractor={item => item.id}
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => {//send the data between two screen which is parcticular list selected to particular data send  
